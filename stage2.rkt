...
(define (get-suffixes text)
  (cond
    [(null? text) '()]
    ; se formeaza lista cu sufixe dupa aplicatia recursiva
...
(define (get-ch-words words ch)
  ...
  (filter (λ (x) (equal? ch (car x)))
          (filter (λ (x) (not (null? x))) words)))
...
(define (ast-func suffixes)
  ; in acc unde deja se afla primul caracter
  ; pun sufixele fara acesta
  (reverse (foldl (λ (x acc) (cons (cdr x) acc))
                  (list (list (caar suffixes))) suffixes)))
...
(define (cst-func suffixes)
  ; in acc unde deja se afla lcpul sufixelor (common)
  ; pun resturile din sufixe (fiecare fara lcp)
  (let ((common (longest-common-prefix-of-list suffixes)))
    (reverse (foldl (λ (x acc) (cons (cadr (longest-common-prefix x common)) acc))
...
(define (suffixes->st labeling-func suffixes alphabet)
  ; modific charurile din lista alfabet in liste cu sufixe
  ; care incep cu charul modificat
  (define (group-by-initial suffixes alphabet)     
    (map (λ (x) (get-ch-words suffixes x)) alphabet))
  ...
  (define (label-and-subtree labeling-func grouped-suffixes)     
    (map labeling-func                                            
       (filter (λ (x) (not (null? x))) grouped-suffixes)))       
